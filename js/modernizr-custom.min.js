!function (a, b, c) {
    ...
    function p(a, b, e, f) {
        function h() {
            l && (delete F.style, delete F.modElem)
        }
        ...
        for (var l, m, n, p, q, r = ["modernizr", "tspan", "samp"]; !F.style && r.length;) l = !0, F.modElem = g(r.shift()), F.style = F.modElem.style;
        for (n = a.length, m = 0; n > m; m++) if (p = a[m], q = F.style[p], j(p, "-") && (p = k(p)), F.style[p] !== c) {
            if (f || d(e, "undefined")) return h(), "pfx" != b || p;
            try {
                F.style[p] = e
            } catch (a) {
            }
            if (F.style[p] != q) return h(), "pfx" != b || p
        }
        return h(), !1
    }

    function q(a, b, c, e, f) {
        var g = a.charAt(0).toUpperCase() + a.slice(1), h = (a + " " + C.join(g + " ") + g).split(" ");
        return d(b, "string") || d(b, "undefined") ? p(h, b, e, f) : (h = (a + " " + D.join(g + " ") + g).split(" "), m(h, b, c))
    }
    ...
    u._prefixes = y, v.addTest("rgba", function () {
        var a = g("a").style;
    ...
    u.mq = A, v.addTest("mediaqueries", A("only all"));
    var B = "Moz O ms Webkit", C = u._config.usePrefixes ? B.split(" ") : [];
    u._cssomPrefixes = C;
    var D = u._config.usePrefixes ? B.toLowerCase().split(" ") : [];
    u._domPrefixes = D;
    var E = {elem: g("modernizr")};
    v._q.push(function () {
        delete E.elem
    });
    var F = {style: E.elem.style};
    v._q.unshift(function () {
        delete F.style
    }), u.testAllProps = q, u.testAllProps = r, v.addTest("csstransitions", r("transition", "all", !0)), e(), f(s), delete u.addTest, delete u.addAsyncTest;
